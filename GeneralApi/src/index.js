...
import {process_setlist_roipage} from "./functions.js";
dotenv.config();
...
app.get('/get/allcardSets/name', async (req, res) => {
    try {
        const cardSets = await cardSet.find({});
        const cardSetNames = cardSets.map(cardSet => cardSet.name);
        res.status(200).send(cardSetNames);
    } catch (error) {
        console.error(error);
        res.status(500).send('Internal Server Error');
    }
...
app.get('/get/rarities', async (req, res) => {
    try {
        const setName = req.query.set;
        console.log(setName);
        if (setName == undefined) {
            res.status(400).send('Invalid set name');
            return;
        }
        let set = await cardSet.find({ name: setName });
        set = set[0];
        const set_rarities = set.get('rarities').sort();

        res.status(200).send(set_rarities);
    } catch (error) {
        console.error(error);
        res.status(500).send('Internal Server Error');
    }
...
app.get('/get/setlist/roipage/details' , async (req, res) => {
    try {
        const setData = await products.find({});
        console.log(setData);
        const setList = process_setlist_roipage(setData);
        console.log(setList);
